# SUMIMASEN — Loss Function and Surface-Priority Metric

> **What this file is:** The formal definition of how SUMIMASEN decides what to surface, how it measures whether its surfacing was useful, and how it improves over time. Grounded in five research papers on intervention timing and proactive AI assistance (see `docs/research/l4-papers.md`).
>
> **Audience:** SUMIMASEN implementation code, the Claude prompt in `mikai_decide.py`, and Brian when he wants to understand or tune SUMIMASEN' behaviour.
>
...
Per candidate (a user need from the registry, or a wiki ## Now thread, or a raw graph thread), SUMIMASEN computes:
...
In V1, **this factor is judged by the LLM** in the prompt because the literature is clear that calibrated `delivery_value` requires a learned model over many interactions, and SUMIMASEN doesn't have those yet. The LLM is given explicit guidance:
...
When SUMIMASEN dispatches a notification, the user can respond in four distinguishable ways. Mapping borrows from PPP's reward function and Inner Thoughts' "participation" stage.

| Signal | Captured how | Maps to |
|---|---|---|
| `ACTED` | (a) ntfy click → opens MIKAI URL handler that posts back; (b) within 24h Brian opens the linked external surface (e.g., Scotia app) and SUMIMASEN sees evidence in the graph; (c) manual `mikai mark-acted <log_id>` CLI | Productivity: +1; Calibration: positive class |
...
**V1 instrumentation gap.** ntfy.sh's iOS app does NOT report click/dismiss back to the publisher. Brian sees the notification but SUMIMASEN sees no response unless Brian uses the CLI. V1 ships with the CLI; V2 should consider native iOS (APNs) or Pushover with click-tracking webhooks. See `SUMIMASEN_SURFACE_DECISION.md`.

---

## 3. The loss function (per tick)

The LLM doesn't see this directly — it's the framework for evaluating SUMIMASEN' decisions over time. Computed daily over the SQLite log:
...
Lower L is better. Negative L = SUMIMASEN is net-helpful.
...
If after 20 dispatches `dismiss_rate > 0.30`, the operational policy is: lower the SUMIMASEN dispatch threshold, NOT increase notification volume. The literature is consistent — surfacing more does not improve action rates, it just trains the user to ignore.

---

## 4. The four PPP-adapted metrics

PPP (CMU Nov 2025, arXiv:2511.02208) ran a joint-optimization training loop on three signals (productivity, proactivity, personalization). SUMIMASEN adapts these for single-user MIKAI plus adds calibration as a fourth quality metric.
...
The most direct measure of whether SUMIMASEN is useful at all. A productivity of 0 = SUMIMASEN is just noise.
...
A proactive surface is one fired by SUMIMASEN' tick logic (vs. one fired by Brian explicitly asking "what should I deal with today?"). Reactive surfaces are easier to make valuable — they answer a direct question. Proactivity isolates whether SUMIMASEN' *unprompted* judgment is good.
...
### 4.4 Calibration — "is SUMIMASEN' stated confidence aligned with actual act rate?"

For each dispatch, SUMIMASEN' decision JSON includes a confidence (implicit in the choice to send + the body language used). We bin dispatches by confidence (low / medium / high) and check whether higher confidence empirically delivers higher act rate.

A simple Brier-score-like metric:

```
calibration_error = mean over all dispatches of (SUMIMASEN_confidence_normalized - actually_acted ? 1 : 0)²
```

Lower is better. Calibration error = 0 means SUMIMASEN' confidence is perfectly aligned with action probability.
...
**V1 implementation:** SUMIMASEN does NOT train a model. Instead, the last 15 decisions are read from SQLite and injected into the Claude prompt verbatim. Claude reads them and adjusts its judgment per-candidate-category:
...
Inner Thoughts (CHI 2025, arXiv:2501.00383) defines a 5-stage loop. SUMIMASEN implements it as:

| Inner Thoughts stage | SUMIMASEN implementation |
|---|---|
...
The **Evaluation** stage is where Inner Thoughts says proactive systems usually fail — they generate plausible candidates but don't gate them well. SUMIMASEN' V1 gate is the explicit "default to silence for noise; only surface stalled or trigger-aligned candidates" prompt instruction. This is a soft gate; the loss-function metrics measure whether the gate is well-calibrated.
...
ALTER TABLE notification_log ADD COLUMN sumimasen_confidence REAL;   -- the priority Claude assigned, 0..1
...
When Brian sees a SUMIMASEN notification and acts on it, he runs `mikai mark-acted` to close the loop. This is friction; V2 replaces it with native iOS click-tracking (Pushover or APNs) or with a Calendar.app integration where the act of opening the linked event itself signals ACTED.

---

## 9. The V2 path — when does SUMIMASEN need real machine learning?
...
## 10. What this file commits SUMIMASEN to
