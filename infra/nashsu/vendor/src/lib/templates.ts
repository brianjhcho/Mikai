...
// MIKAI-specific template: fork of Personal Growth adding a `wisdom` page type
// for aggregating one-line insights and aphorisms from journals that contain
// many short items which don't individually warrant a concept page but
// collectively represent accumulated wisdom.
// MIKAI deviation from upstream nashsu — added 2026-08-20 (Session-7 plan).
const mikaiTemplate: WikiTemplate = {
  id: "mikai",
  name: "MIKAI",
  description: "Personal Growth extended with a wisdom page type for aggregating one-line insights",
  icon: "🧭",
  extraDirs: ["wiki/goals", "wiki/habits", "wiki/reflections", "wiki/journal", "wiki/wisdom"],
  schema: `# Wiki Schema — MIKAI
...
| reflection | wiki/reflections/ | Periodic reviews and lessons learned |
| journal | wiki/journal/ | Freeform daily or session entries |
| wisdom | wiki/wisdom/ | Collections of one-line insights and aphorisms, grouped by theme |
...
- Reflections: type + date (e.g., \`weekly-2024-03.md\`, \`quarterly-2024-q1.md\`)
- Journal: date slug (e.g., \`2024-03-15.md\`)
- Wisdom: theme slug (e.g., \`ganas-and-determination.md\`, \`passion-vs-contribution.md\`)
...
- Goals link to the habits that support them via \`related:\`
- Journal entries can reference goals and reflections inline with \`[[slug]]\`
- Wisdom pages link OUT to concept pages that name their theme; concept pages reciprocally reference the wisdom page in their \`related:\` frontmatter
...
Journals in this vault often contain many short one-line insights or aphorisms (5+ per source). Route these to wisdom pages rather than fragmenting each into a separate concept.

**When to create a wisdom page:** any source containing 5+ one-line insights on a shared theme. If the source has fewer than 5 aphorisms, they may be embedded in the source-summary page instead.

**Wisdom page format:**
- Body opens with a 1-2 sentence intro describing the theme (in English)
- Each entry is a blockquote with attribution and thematic wikilink:
  \`\`\`
  > "quote text — verbatim from the source, in the source's original language"
  > — from [[sources/source-slug]], theme [[concepts/concept-slug]]
  \`\`\`
- When a quote is not in English, immediately follow it with an English translation on the next blockquote line:
  \`\`\`
  > "야망은 크고 손은 작은"
  > "Ambitions big, hands small" *(translated)*
  > — from [[sources/source-slug]], theme [[concepts/concept-slug]]
  \`\`\`
- Attribution is mandatory: every quote cites BOTH the source page AND the theme concept via wikilinks
- Group quotes under \`## Section\` headers when a wisdom page carries multiple sub-themes

**Concept-page reciprocity:**
- When a concept has 2+ quotes on a wisdom page, add a \`## Notable Quotes\` section to the concept page
- The section shows 2-4 sample quotes verbatim with attribution back to the source
- Close the section with: \`See [[wisdom/wisdom-page-slug]] for the full collection.\`
- The concept's \`related:\` array must include the wisdom page slug

**Cross-source accumulation:**
- If a new source contributes 3+ quotes to a theme that already has a wisdom page, append to the existing wisdom page rather than creating a duplicate
- Preserve chronological order within each section (oldest quote first, unless a stronger thematic grouping exists)
`,
  purpose: `# Project Purpose — MIKAI
