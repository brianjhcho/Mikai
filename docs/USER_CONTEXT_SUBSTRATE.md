...
## 4. Migration shipped — and what was canceled with it

*2026-08-07. Deliverable 1 landed: `infra/mikai_brain/user_model.py` collapsed to two sections (`durable`, `current`), 2,048-byte cap with `RuntimeError` on overflow, `source_signals` routed to `~/.mikai/brain/state/user_model_signals.log` instead of the injected markdown. `hydrator.py` and `latent_threads.py` now score against `um.current` (the merged themes-plus-unresolved slot); `mikai_ask/core.py`'s injection order is unchanged — the file just shrank. Live rebuild produced a 1,815-byte USER_MODEL.md with 6 durable + 6 current items. Prior `USER_MODEL.md` and `user_model.json` were backed up as `.bak-<ts>` before overwrite.*

**Deliverables 2 and 3 canceled on operator direction.** The next step in the original plan added four dream-weekly subcommands (`obsessions`, `aphorisms`, `ideas`, `expertise`) that would each generate a precomputed wiki page (`~/.mikai/wiki/wiki-<name>.md`), plus retrieval hooks in `mikai_ask` and the Attention Engine to inject those pages. Brian correctly flagged this as the exact anti-pattern §1-§2 dismantled: precomputed taxonomies of inductive content are "wiki content wearing a model costume." Building them under the wiki roof rather than the USER_MODEL roof does not repair the mistake — it just relocates it, and adds four more interactive-tier LLM calls per dream-weekly to maintain files that the corpus itself already contains.

The right substitute is **query-time synthesis via the existing `mikai_ask` pipeline**, which already reads the whole wiki through FTS + windowed fill. Questions like "what does Brian keep coming back to?" or "what has he said about breathing?" are ask-time inductions over the corpus; they do not need a precomputed page to exist. For the specific "tensions" cockpit view Brian mentioned separately, the parser will read `## Tensions` sections that already exist in `wiki.md` directly — no dream operation required.

Net effect of the reconciliation: **one file smaller, one file simpler, zero new files, zero new LLM operations on the schedule.** The literature convergence in §2 pointed at deletion, and deletion is what shipped.
