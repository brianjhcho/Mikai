...
#### Hierarchical Summarization for Low-Density Sources

Per-conversation-window segmentation is not enough for chat-density sources. A WhatsApp thread with Martin about a Kenya coffee farm produces hundreds of windows over weeks; embedding all of them into Graphiti both inflates ingestion cost and dilutes retrieval — the signal in any single window is low.

The pattern, drawn from GAM's "event progression graph → topic associative network" (arXiv:2604.12285) and now widely emerging as best practice across chat-memory systems:

```
level 0 (raw):      every message, stored but never sent to the LLM alone
level 1 (daily):    "2026-04-18: Martin shared photos of new coffee plots,
                     discussed yield estimates"
level 2 (topical):  "Martin + Brian thread on Kenya coffee farm:
                     Apr 10 (introduction) → Apr 15 (farm tour pics) →
                     Apr 18 (data email)"
```

Only level 1+ becomes a Graphiti episode. Level 0 stays in an append-only log on disk for potential drill-down — addressable by `(source_id, timestamp_range)`, not loaded by default. Level 2 is generated by a periodic consolidation pass when a topic accumulates enough level-1 entries (threshold: 3+ daily summaries within a 14-day window for the same `(participants, topic_label)` tuple).

This implements mechanism M7 (boundary-triggered consolidation) from `docs/MEMORY_ARCHITECTURE_THESIS.md` for the chat-density case specifically. The level-0 log preserves ground truth (MemMachine pattern: don't over-process at ingest); the level-1/2 summaries are what L4 thread detection actually queries against.

Cost profile: one Haiku call per active conversation per day for level-1, one Haiku call per topic-shift event for level-2. For a typical user this is a few dollars per month, vs hundreds of dollars to push raw messages through Graphiti's full extraction.

**Density tier matrix (corpus-wide, drives splitter selection):**

| Density tier | Signal per 1000 tokens | Examples | Right treatment |
|---|---|---|---|
| High (reflection / research) | 1+ insight | Claude threads, long Apple Notes, processed reflections, research PDFs | Semantic chunking; full LLM extraction; high confidence on extracted facts |
| Medium (correspondence) | 1 insight per few messages | Emails with substance, structured DMs | Thread-aware splitting (reply boundaries); extract entities + commitments |
| Low (chat messages) | 1 insight per dozens | WhatsApp banter, iMessage, Slack noise | Hierarchical summarization (above) — summarize windows, keep recent verbatim, extract only when density spikes |
