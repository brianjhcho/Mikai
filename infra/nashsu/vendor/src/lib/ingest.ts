...
export function parseFileBlocks(text: string): ParseFileBlocksResult {
  // MIKAI deviation (2026-08-21, Session-7): the LLM sometimes emits a
  // horizontal-rule '---' on its own line immediately BEFORE the
  // 'END FILE---' or 'FILE:' marker, splitting a closer/opener across
  // two lines. Nashsu's single-line regex misses it and swallows all
  // subsequent pages into the current block. Rejoin the split before
  // any other parsing runs. See June 2026 ingest data-loss incident.
  const rejoined = text
    .replace(/^---[ \t]*\r?\n(?=END\s+FILE---\s*$)/gm, "---")
    .replace(/^---[ \t]*\r?\n(?=FILE:\s)/gm, "---")
  ...
  const normalized = rejoined.replace(/\r\n/g, "\n")
...
async function autoIngestImpl(
  ...
  // MIKAI mechanism-B link-at-birth: prepend semantic neighbors to the
  // wiki index so the extraction LLM sees the top-K semantically similar
  // existing pages first, before scanning the full lexical directory.
  // The CLI (infra/nashsu/cli/ingest.ts) writes these via a pre-extraction
  // hook that runs pre_extract_topk.py. Silent-degrade to lexical-only
  // if the neighbors file is absent or unreadable.
  const neighborsPath = `${pp}/wiki/.mikai-neighbors/${fileName}`
  const semanticNeighbors = await readFile(neighborsPath).catch(() => "")
  const indexWithNeighbors = semanticNeighbors
    ? `${semanticNeighbors.trim()}\n\n${index}`
    : index
  ...
  if (!analysis) {
    await streamChat(
      llmConfig,
      [
        { role: "system", content: buildAnalysisPrompt(purpose, indexWithNeighbors, sourceContext, schema) },
  ...
  await streamChat(
    llmConfig,
    [
      { role: "system", content: buildGenerationPrompt(schema, purpose, indexWithNeighbors, sourceIdentity, overview, sourceContext, sourceSummaryPath) },
      {
        role: "user",
        content: [
...
async function writeFileBlocks(
  ...
  for (const { path: rawRelativePath, content: rawContent } of blocks) {
    ...
    const isEntityOrSource =
      ...
      // MIKAI deviation (2026-08-20, Session-7): wisdom pages legitimately
      // cite source-verbatim aphorisms in whatever language the source used.
      // Same rationale as the existing sources/entities exemption above.
      relativePath.startsWith("wiki/wisdom/") ||
      relativePath.includes("/wisdom/")
    if (
      targetLang &&
      targetLang !== "auto" &&
      !isLog &&
...
export function buildAnalysisPrompt(
  ...
  return [
    ...
    ...(process.env.MIKAI_NO_SLUG_DISCIPLINE === "1" ? [] : [
      "## SLUG DISCIPLINE (LOAD-BEARING RULE)",
      "The Current Wiki Index at the end of this prompt lists every existing entity, concept, and wisdom page in the knowledge base.",
      "For every entity/concept you extract:",
      "1. FIRST scan the index for an existing slug that matches OR is a synonym of the item (e.g. 'information-asymmetry' ↔ 'asymmetric-information', 'personal-intent-graph' ↔ 'intent-graph', 'extended-mind' ↔ 'extended-mind-thesis').",
      "2. IF you find a match: REUSE the existing slug verbatim. Do NOT coin a new synonym slug.",
      "3. ONLY invent a new slug when the item is genuinely NEW to the wiki (no existing entry, no synonym).",
      "Slug proliferation is a bug. Mature knowledge bases converge on fewer, denser pages — new-slug rate should FALL over time (Heaps' law).",
      "",
    ]),
    ...
    ...(process.env.MIKAI_NO_SLUG_DISCIPLINE === "1" ? [] : ["- SLUG DECISION: existing slug from the index (name it explicitly) OR 'GENUINELY NEW' (justify why no existing slug fits)"]),
    "",
    "## Key Concepts",
...
export function buildGenerationPrompt(
  ...
  return [
    ...
    "",
    ...(process.env.MIKAI_NO_SLUG_DISCIPLINE === "1" ? [] : [
      "## SLUG DISCIPLINE (LOAD-BEARING RULE — same as stage 1)",
      "The Current Wiki Index at the end of this prompt lists every existing entity, concept, and wisdom page.",
      "The analysis you were given should have made a SLUG DECISION for each entity/concept: either an existing slug from the index OR 'GENUINELY NEW'.",
      "When you generate FILE blocks:",
      "- If the analysis assigned an EXISTING slug, write to that existing path (the write path merges frontmatter arrays and asks the LLM for body merge). Do NOT create a new file with a synonym slug.",
      "- If the analysis assigned GENUINELY NEW, write a new FILE block with a new slug.",
      "- Body wikilinks in any generated page should use existing slugs from the index whenever the referenced concept exists there — don't invent synonyms in `[[…]]` targets either.",
      "Slug proliferation is a bug. Fewer, denser pages > many synonym pages.",
      "",
    ]),
