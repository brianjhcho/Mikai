# SUMIMASEN — Notification Surface Decision
...
Brian invoked the iPhone metaphor: collect all life needs in one place with AI managing them. The single-surface goal is real but a single-surface V1 is premature — Brian has different attention modes in different contexts (mobile vs laptop, focused work vs ambient). The right framing is *cohesive multi-surface, single brain* — SUMIMASEN makes one decision, dispatches to whichever surface fits the moment.

A useful constraint: any V1 surface must be **already-running** for Brian. He's not going to install a new app just to receive SUMIMASEN notifications. He IS already running:
...
For each surface: latency, interactivity, persistence, batching, infra cost, fit, Steve-Jobs metaphor fit, SUMIMASEN impl path.
...
|---|---|
| Latency | <2s globally |
| Interactivity | Limited — tap opens body, no inline buttons reaching SUMIMASEN without a webhook handler |
...
| SUMIMASEN impl | `dispatch_via_ntfy(title, body, priority)` already in `mikai_decide.py` |

**Verdict for V1:** Keep. Primary mobile surface. Brian sees these whether or not he's at his laptop.

**Known limitation:** No click-tracking back to SUMIMASEN. The CLI workaround (`mikai mark-acted`) is the V1 closer-of-loop.
...
| SUMIMASEN impl | `dispatch_calendar.py` writes the event via osascript at the start of each day. Body = the top-3 from SUMIMASEN' candidate slate. |
...
| SUMIMASEN impl | Trivial: `osascript -e 'display notification "..." with title "..."'` |
...
| SUMIMASEN impl | `subprocess.run(["terminal-notifier", "-title", t, "-message", b, "-actions", "..."])` returns Brian's choice → write straight to log |

**Verdict for V1:** Wire up but graceful fall-through to osascript if not installed. Brian's machine doesn't have it yet (as of this session). Prompt him once to install; SUMIMASEN works without it.
...
| SUMIMASEN impl | Replace `dispatch_via_ntfy()` with `dispatch_via_pushover()` — same shape, different HTTP endpoint and auth |
...
| SUMIMASEN impl | Extension polls `mikai_decide.py --json --show-slate` every N minutes; rich UI in popup. |
...
|---|---|
| Latency | <1s |
| Interactivity | Inline keyboard buttons (Acted/Dismiss/Snooze) reach SUMIMASEN via webhook |
| Persistence | Chat history |
| Batching | Per-event or batched (SUMIMASEN controls) |
...
| SUMIMASEN impl | python-telegram-bot library + a small webhook server (already have Pattern B infra) |
...
| SUMIMASEN impl | SMTP via the existing Gmail adapter, swapped from read to send |

**Verdict for V1:** Skip. Email is high-noise; SUMIMASEN' job is to reduce, not add. V3 candidate if Brian wants a weekly digest.
...
| SUMIMASEN impl | Raycast Node extension calling `mikai_decide.py --json --show-slate` |
...
| SUMIMASEN impl | Swift menubar app polling `mikai_decide.py --json --show-slate` |
...
2. **macOS Calendar.app daily brief (primary persistent surface)** — `dispatch_calendar.py` writes a 7am daily event with the top-3 candidates from SUMIMASEN' slate
3. **macOS osascript notification (immediate-attention secondary)** — when SUMIMASEN fires a notification, also pop a macOS native one for low-friction Brian-at-desk awareness
...
These four surfaces share one SUMIMASEN brain. The decision of what to surface happens once per tick; dispatch fans out.
...
Jobs collected the camera, iPod, and phone into one device because the user's *attention is one*. SUMIMASEN collects all of Brian's life needs into *one decision* — the same brain decides what to surface across iPhone, Calendar, Mac notification, and (later) browser extension.

The brain is `mikai_decide.py` reading the User Needs Registry + Dream wiki + adapters. The surfaces are dumb dispatchers. This separation is the Jobs unification — the user experience is "MIKAI knows what I care about and surfaces it where I'll see it." The actual delivery channel is implementation detail SUMIMASEN chooses based on context.
...
| Channel | When SUMIMASEN uses it | Code path |
|---|---|---|
...
2. Next SUMIMASEN tick: receives an ntfy notification AND a macOS notification AND a Calendar event for tomorrow.
3. Brian reads the Calendar event tomorrow morning; sees the top-3 brief.
4. Brian either runs `mikai mark-acted <id>` or lets it sit; SUMIMASEN observes via `acted_within_24h` backfill.
5. After 7 days: `mikai metrics` is meaningful. Brian + SUMIMASEN together evaluate whether V2 is warranted.
